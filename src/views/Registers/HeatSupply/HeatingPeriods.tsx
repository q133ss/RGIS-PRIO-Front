...
interface HeatingScheduleItem { 
  ...
  created_at?: string; 
  updated_at?: string; 
  mkd?: MkdBuilding | null | undefined; 
  address?: Address | null | undefined;
  // Old order fields (for backwards compatibility)
  ...
  // New order objects
  connect_order?: OrderItem | null;
  disconnect_order?: OrderItem | null;
}

interface HeatingScheduleFormData { 
  mkd_id: number | ''; 
  address_id: number | ''; 
  status: boolean; 
  planned_disconnection_date: string | null; 
...
const HeatingSchedulePage: React.FC = () => {
  ...
  const handleShowForm = async (item?: HeatingScheduleItem) => {
    ...
    if (item) {
      try {
        ...
        // Map the order data from the nested objects or flat fields
        setFormData({
    ...
    try {
      ...
      const filteredItems = response.data.filter(item => {
        ...
        return address.toLowerCase().includes(searchInput.toLowerCase()) ||
          ...
          (connectNumber && connectNumber.toLowerCase().includes(searchInput.toLowerCase()));
      });
      setState(prev => ({
        ...prev,
        items: filteredItems,
  ...
  const sortData = (data: HeatingScheduleItem[]): HeatingScheduleItem[] => {
    return [...data].sort((a, b) => {
      ...
      if (sortField === 'disconnection_omsu_order_number') {
        const aValue = a.disconnect_order?.number || a.disconnection_omsu_order_number || '';
        const bValue = b.disconnect_order?.number || b.disconnection_omsu_order_number || '';
        return sortDirection === 'asc'
          ? String(aValue).localeCompare(String(bValue), 'ru')
          : String(bValue).localeCompare(String(aValue), 'ru');
      }
      
      if (sortField === 'connection_omsu_order_number') {
        const aValue = a.connect_order?.number || a.connection_omsu_order_number || '';
        const bValue = b.connect_order?.number || b.connection_omsu_order_number || '';
        return sortDirection === 'asc'
          ? String(aValue).localeCompare(String(bValue), 'ru')
          : String(bValue).localeCompare(String(aValue), 'ru');
      }
          ...
          cellContent = orderNumber ? (
            <OverlayTrigger overlay={
              <Tooltip id={`tooltip-disc-${item.id}`}>
                {orderTitle || 'Order'}
                {orderDate && ` от ${formatDate(orderDate)}`}
              </Tooltip>
            }>
              <span className="text-primary cursor-pointer">{orderNumber}</span>
            </OverlayTrigger>
          ) : ('-');
            ...
            <>
              ...
              <Row>
                <Col md={6}>
                  ...
                  <p className="mb-2">
                    <span className="fw-bold">Номер:</span> {currentItem.disconnect_order?.number || currentItem.disconnection_omsu_order_number || '-'}
                  </p>
                  <p className="mb-2">
                    <span className="fw-bold">Дата:</span> {formatDate(currentItem.disconnect_order?.date || currentItem.disconnection_omsu_order_date)}
                  </p>
                  <p className="mb-2">
                    <span className="fw-bold">Название:</span> {currentItem.disconnect_order?.title || currentItem.disconnection_omsu_order_title || '-'}
                  </p>
                  <p className="mb-2">
                    <span className="fw-bold">Доп. инфо:</span> {currentItem.disconnect_order?.additional_info || currentItem.disconnection_omsu_order_additional_info || '-'}
                  </p>
                </Col>
                <Col md={6}>
                  <h6 className="fw-bold mb-3">Приказ о включении</h6>
                  <p className="mb-2">
                    <span className="fw-bold">Номер:</span> {currentItem.connect_order?.number || currentItem.connection_omsu_order_number || '-'}
                  </p>
                  <p className="mb-2">
                    <span className="fw-bold">Дата:</span> {formatDate(currentItem.connect_order?.date || currentItem.connection_omsu_order_date)}
                  </p>
                  <p className="mb-2">
                    <span className="fw-bold">Название:</span> {currentItem.connect_order?.title || currentItem.connection_omsu_order_title || '-'}
                  </p>
                  <p className="mb-2">
                    <span className="fw-bold">Доп. инфо:</span> {currentItem.connect_order?.additional_info || currentItem.connection_omsu_order_additional_info || '-'}
                  </p>
                </Col>
              </Row>
            </>
